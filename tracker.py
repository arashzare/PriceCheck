...
def run_tracker():
    # Read target threshold from env or default to 420 CAD (~$310 USD, around user's $400 budget)
    ...
    # 1. Scrape with Playwright (Amazon CA & Amazon Global)
    try:
        print("\n[Playwright] Launching browser for Amazon CA / Global...")
    ...
    print("\n--- Current Lowest Available Market Listings (All Landed to CA) ---")
    if unique_deals:
    ...
    # Filter deals meeting the target price threshold
    ...
    with open("latest_deals_report.md", "w", encoding="utf-8") as f:
        ...
        f.write(f"- **Target Threshold:** ${target_price_cad:.2f} CAD (all shipping & import fees included)\n")
        ...
        if matching_deals:
            for d in matching_deals:
                ...
                f.write(f"- **Item Price:** ${d.item_price_cad:.2f} CAD\n")
                ...
                f.write(f"- **Listing Title:** {d.title}\n")
                f.write(f"- **Direct Link:** [View & Purchase on {d.store}]({d.url})\n\n")
        else:
        ...
        if unique_deals:
            for d in unique_deals[:10]:
                f.write(f"- **{d.model}** ({d.store}): **${d.total_price_cad:.2f} CAD** (Item: ${d.item_price_cad:.2f} + Ship: ${d.shipping_price_cad:.2f}) — [Link]({d.url})\n")
        else:
